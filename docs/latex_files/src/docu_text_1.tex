...
In the table above, the characteristics of PitchApp are collected to show how it fulfills the project requirements.
...
The following figures show how a full window-size version and a small window-size version of the landing page of PitchApp's UI look with the reorganized UI elements.
...
The public section of PitchApp is comprised of a single web page that guests visit when they want to use the application. This web page presents the main idea behind PitchApp, information for idea owners and idea sponsors, as well as our privacy policy about how PitchApp protects the ownership of ideas. Finally, it includes a button to log in with Okta. Once the user logs in with his or her email address as user name and password, he/she can access the rest of the functionality offered by PitchApp, depending on whether the user is an idea owner or a sponsor. Please note that only for testing and evaluation purposes a third user type called super user was added, which has both sponsor and idea owner rights.
...
Pitch deletion can be done by clicking on a button and can be performed only by the user who created the pitch. With this check, we ensured that none else can delete a pitch except the idea owner him/herself. Before deleting a pitch, the idea owner is asked again "Are you sure?". With building in this additional question, we wanted to avoid accidental deletions.
...
\chapterauthor{by Stella Kamakari and Boglarka Lehoczki}
...
\textbf{https://github.com/web-programming-dhbw/react-app}
...
\begin{center}
	\includegraphics{pitch_table.png}
\end{center}
...
\begin{center}
	\includegraphics{hasura_header.png}
\end{center}
...
\item To login, the user can click on the “Login” button.
...
The creator of a pitch may also delete it by pressing the red “Delete Pitch” button that also appears inside the pitch’s card. To avoid not-intended deletions, the idea owner is asked again by the red button. Clicking on "Are you sure?" deletes the pitch from the system.
