...
\chapter{User Manual - Development Insight into Running PitchApp}

\chapterauthor{by Stella Kamakari}

\subsection{Running PitchApp}

For PitchApp, the optimal way to run the whole application is through Docker. We have provided a docker-compose file that takes all of the images that are needed and runs them on the proper ports. As it was discussed before the images that are taken for HasuraGraphQL Engine and Postgres are official ones. However, the one for PitchApp is our own that was uploaded to DockerHub with the account of Csaba Kegyes. 
Using the command line the following command should be ran from the folder where our docker-compose.yaml file is located:

docker-compose up

This command will automatically download the images and start the application. The front-end can be reached at localhost:8000. When trying this method, the table, that we have described in the section detailing Postgres, has to be created for the application to work. This can be done in the Hasura interface that can be reached at localhost:8080.

We also have a working site that can be reached at: URL. Also, an instance of the database interface can be reached at URL. \todo{add urls}

In order to run the react front-end, navigate to the 'app' folder in our repository. First the dependencies should be installed. This can be done by the following command:

npm install

After this is done the application can be run in developer mode by running the next command:

npm start

It will open localhost:3000 with PitchApp.

Our React app can also be put into production ready mode by running the following command:

npm run build

This creates a build folder in the same directory that can be served by the build serve method or by using other servers, such as Nginx.

After the build process is done the React application can be put into a container. There is already a Docker file that specifies how this should be done using a base Nginx image. Run the following command in the command line:

docker build -t "NAME OF IMAGE" .

The name of the image in our case is pitch-docker.

This image can be used separately, as well as the developer and production builds, but they will be missing the database connection.
During development we started the database connection from one terminal with
docker-compose up
and from another terminal we opened the development view of PitchApp with
